...
export default function Kalender() {
  ...
  return (
    <div className="h-screen flex flex-col bg-slate-50 dark:bg-slate-950 overflow-hidden">
      ...
      {/* ── MOBILE HEADER ── */}
      <div className="lg:hidden bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 px-4 py-2 flex flex-col gap-2">
        {/* Row 1: title + new event */}
        <div className="flex items-center justify-between">
          <h1 className="text-lg font-bold text-slate-900 dark:text-white">Kalender</h1>
          <button
            onClick={() => { setFormData({ ...formData, start_time: `${format(new Date(), 'yyyy-MM-dd')}T09:00`, end_time: `${format(new Date(), 'yyyy-MM-dd')}T10:00` }); setShowDialog(true); }}
            className="h-9 w-9 rounded-full bg-emerald-600 flex items-center justify-center shadow-md">
            <Plus className="h-5 w-5 text-white" />
          </button>
        </div>
        {/* Row 2: nav + period */}
        <div className="flex items-center justify-between gap-2">
          <button onClick={navigatePrev} className="p-1.5 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800">
            <ChevronLeft className="h-5 w-5 text-slate-600 dark:text-slate-400" />
          </button>
          <button onClick={goToToday} className="flex-1 text-center text-sm font-semibold text-slate-700 dark:text-slate-300 truncate">
            {getPeriodLabel()}
          </button>
          <button onClick={navigateNext} className="p-1.5 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800">
          ...
          {['day','week','month','year'].map((v) => (
            <button key={v} onClick={() => setCalendarView(v)}
              className={cn("flex-1 py-1.5 rounded-lg text-xs font-medium transition-colors capitalize",
                calendarView === v ? "bg-white dark:bg-slate-700 text-slate-900 dark:text-white shadow-sm" : "text-slate-500 dark:text-slate-400")}>
              {v === 'day' ? 'Dag' : v === 'week' ? 'Uke' : v === 'month' ? 'Måned' : 'År'}
            </button>
          ))}
        </div>
      </div>
      ...
      {/* ── DESKTOP HEADER ── */}
      <div className="hidden lg:flex bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 px-8 py-3 items-center justify-between gap-3 flex-wrap">

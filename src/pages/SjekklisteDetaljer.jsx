...
import {
  ...
  PenLine, Download, Mail, History, FileText
} from 'lucide-react';
...
export default function SjekklisteDetaljer() {
  ...
  const handleStatusChange = (itemIndex, status) => {
    ...
    const activity_log = logActivity(`Status endret på "${itemTitle}"`, `Ny status: ${statusLabel(status)}`);
  ...
  const handleCreateDeviation = async (itemIndex) => {
    ...
    const activity_log = logActivity(`Avvik opprettet fra "${itemTitle}"`, `Avviks-ID: ${deviation.id}`);
    updateMutation.mutate({ responses, activity_log });
  ...
  if (error) return (
    <div className="flex items-center justify-center min-h-screen p-4">
      <div className="text-center max-w-md">
        ...
        <p className="text-sm text-slate-600 mb-4">Prøv å gå tilbake og åpne den på nytt.</p>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 p-4 md:p-6 pb-20 md:pb-6">
      <div className="max-w-3xl mx-auto">
        ...
        <Card className="bg-white p-5 mb-6 shadow-sm">
          ...
          <div className="flex flex-wrap gap-2">
            {!isComplete && (
              <Button onClick={handleComplete} disabled={controlledItems === 0} className="gap-2 bg-emerald-600 hover:bg-emerald-700 text-sm">
                <Check className="h-4 w-4" /> Fullfør
              </Button>
            )}
            ...
            <Button variant="outline" onClick={() => generateChecklistPDF(checklist, project?.name)} className="gap-2 text-sm">
              <Download className="h-4 w-4" /> PDF
            </Button>
            <Button variant="outline" onClick={() => setShowEmailDialog(true)} className="gap-2 text-sm">
              <Mail className="h-4 w-4" /> Send
            </Button>
            <Button variant={editMode ? 'default' : 'outline'} onClick={() => setEditMode(!editMode)} className="gap-2 text-sm">
              ...
              {(checklist.sections || []).length > 0 ? (
                <>
                  {checklist.sections.map((section, secIdx) => {
                    const globalOffset = checklist.sections.slice(0, secIdx).reduce((sum, s) => sum + (s.items?.length || 0), 0);
                    return (
                      <div key={secIdx}>
                          ...
                          <div className="flex-1">
                            {editMode ? (
                              <div className="space-y-2">
                                <Input value={section.title} onChange={(e) => { const s = [...checklist.sections]; s[secIdx].title = e.target.value; updateMutation.mutate({ sections: s }); }} className="text-lg font-bold" />
                                <Textarea value={section.description || ''} onChange={(e) => { const s = [...checklist.sections]; s[secIdx].description = e.target.value; updateMutation.mutate({ sections: s }); }} placeholder="Beskrivelse" rows={2} />
                              </div>
                            ) : (
                              <>
      ...
      <ChecklistSignDialog open={showSignDialog} onOpenChange={setShowSignDialog} onSign={handleSign} existingSignatures={checklist.signatures || []} user={user} />
      <ChecklistSendEmailDialog open={showEmailDialog} onOpenChange={setShowEmailDialog} checklist={checklist} project={project} />
    </div>
  );
}
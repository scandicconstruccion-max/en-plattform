  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      {selectedDeviations.length > 0 && (
        <div className="bg-emerald-600 border-b border-emerald-700 px-4 lg:px-8 py-2.5">
          <div className="flex flex-wrap items-center gap-2">
            <span className="text-sm font-semibold text-white whitespace-nowrap bg-emerald-800/40 px-2.5 py-1 rounded-lg flex-shrink-0">
              {selectedDeviations.length} valgt
            </span>
            <div className="w-px h-5 bg-emerald-400/50 flex-shrink-0" />
            <Button onClick={() => setShowBulkEmailDialog(true)} variant="ghost" size="sm" className="rounded-lg gap-1.5 text-white hover:bg-emerald-700 h-8 px-3 whitespace-nowrap">
              <Send className="h-3.5 w-3.5" /> Send e-post
            </Button>
            <Button onClick={() => setShowStatusDialog(true)} variant="ghost" size="sm" className="rounded-lg gap-1.5 text-white hover:bg-emerald-700 h-8 px-3 whitespace-nowrap">
              <CheckCircle2 className="h-3.5 w-3.5" /> Endre status
            </Button>
            <Button onClick={handleBulkDownload} variant="ghost" size="sm" className="rounded-lg gap-1.5 text-white hover:bg-emerald-700 h-8 px-3 whitespace-nowrap">
              <Download className="h-3.5 w-3.5" /> Last ned PDF
            </Button>
            <div className="w-px h-5 bg-emerald-400/50 flex-shrink-0" />
            <Button onClick={() => setShowDeleteDialog(true)} variant="ghost" size="sm" className="rounded-lg gap-1.5 text-red-200 hover:text-white hover:bg-red-600 h-8 px-3 whitespace-nowrap">
              <Trash2 className="h-3.5 w-3.5" /> Slett
            </Button>
            <Button onClick={() => setSelectedDeviations([])} variant="ghost" size="sm" className="rounded-lg text-emerald-100 hover:text-white hover:bg-emerald-700 h-8 px-3 whitespace-nowrap">
              Avbryt
            </Button>
          </div>
        </div>

...
export default function UploadQuoteDialog({ open, onOpenChange, customers }) {
  ...
  const createMutation = useMutation({
    mutationFn: async (data) => {
      ...
      return base44.entities.QuoteFollowUp.create({
        ...
        external_system_reference: 'manual_upload'
      });
    },
    onSuccess: () => {
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      ...
      setFormData({
        ...formData,
        documents: [...formData.documents, { name: file.name, url: file_url }]
      });
    } catch (error) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Last opp tilbud fra PC</DialogTitle>
        </DialogHeader>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <Label>Last opp tilbudsdokument</Label>
            <div className="mt-1.5">
              <label className="flex items-center gap-2 px-4 py-2 border-2 border-dashed rounded-xl cursor-pointer hover:bg-slate-50 transition-colors">
                ...
                <span className="text-sm text-slate-600">
                  {uploading ? 'Laster opp...' : 'Klikk for å laste opp'}
                </span>
                <input
                  type="file"
          ...
          <div className="flex justify-end gap-3 pt-4">
            ...
            >
              {createMutation.isPending ? 'Legger til...' : 'Legg til tilbud'}
            </Button>
          </div>
        </form>
      </DialogContent>

...
export default function SendRUHDialog({ open, onOpenChange, selectedRuh, ruhList, projects }) {
  ...
  const sendMutation = useMutation({
    mutationFn: async (emailList) => {
      const promises = emailList.map(async (email) => {
        for (const ruhId of selectedRuh) {
          const ruh = ruhList.find(r => r.id === ruhId);
          const project = projects.find(p => p.id === ruh?.project_id);
          
          await base44.integrations.Core.SendEmail({
            to: email,
            subject: `RUH - ${project?.name || 'Prosjekt'}`,
            body: `
              <h2>Rapport om uønsket hendelse</h2>
              <p><strong>Prosjekt:</strong> ${project?.name || 'Ukjent'}</p>
              <p><strong>Dato:</strong> ${ruh.dato}</p>
              <p><strong>Hva skjedde:</strong> ${ruh.hva_skjedde || 'Ingen beskrivelse'}</p>
              <p><strong>Status:</strong> ${ruh.status}</p>
              <br>
              <p>Se vedlagt RUH-rapport for mer informasjon.</p>
            `
          });
        }
      });
    ...
    onSuccess: () => {
      toast.success(`RUH-rapport sendt til ${recipients.length} mottaker(e)`);
      setRecipients([]);
      setEmailInput('');
      setSelectedEmployee('');
      onOpenChange(false);
  ...
  const handleSend = () => {
    ...
    const projectNames = [...new Set(selectedRuh.map(id => {
      const ruh = ruhList.find(r => r.id === id);
      const project = projects.find(p => p.id === ruh?.project_id);
      return project?.name || 'Ukjent';
    }))].join(', ');

    toast.info(`RUH-skjema for prosjekt ${projectNames} sendes på nytt...`);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>Send RUH-rapport på nytt</DialogTitle>
        </DialogHeader>

        <div className="space-y-4 py-4">
          <p className="text-sm text-slate-600">
            Sender {selectedRuh.length} RUH-rapport(er)
          </p>

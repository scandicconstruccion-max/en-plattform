...
// Tellerne har en EGEN kilde (RPC-en), ikke fillistas spørring. Da 24bf0ff
// strammet lista, ble tellerne stående igjen og summerte alle faser: én tegning
// kopiert til tre faser ga «3 filer» over en liste med én, i hver av de tre.
console.log('\n── Kategoritellerne ──')

// Kjør den EKTE summeringen fra useMemo-en mot konstruerte RPC-rader.
const memoStart = src.indexOf('const { catCounts, subCounts } = React.useMemo(')
if (memoStart === -1) {
  kritisk('Fant ikke useMemo-en som utleder catCounts/subCounts')
} else {
  const kropp = src.slice(src.indexOf('{', src.indexOf('=> {', memoStart)) + 1, src.indexOf('return { catCounts: cat, subCounts: sub }', memoStart))
  // Kroppen deklarerer selv cat og sub — vi returnerer dem bare ut.
  const summer = new Function('countRows', 'hasFaser', 'viewedPhase',
    kropp + 'return { cat, sub };')

  // Samme tegning kopiert til tre faser — det observerte tilfellet.
  const rader = [
    { phase: 'anbud',     category: 'tegninger', sub_folder: 'Arkitekttegninger', doc_type: null, antall: 1 },
    { phase: 'kontrakt',  category: 'tegninger', sub_folder: 'Arkitekttegninger', doc_type: null, antall: 1 },
    { phase: 'utforelse', category: 'tegninger', sub_folder: 'Arkitekttegninger', doc_type: null, antall: 1 },
    { phase: null,        category: 'tegninger', sub_folder: 'Arkitekttegninger', doc_type: null, antall: 1 },
    { phase: 'anbud',     category: 'okonomi',   sub_folder: null,                doc_type: 'tilbudsbrev', antall: 1 },
  ]

  const iUtforelse = summer(rader, true, 'utforelse')
  sjekk('Utførelse: tegninger teller egen fase + uten fase', iUtforelse.cat.tegninger, 2)
  sjekk('Utførelse: undermappe teller likt', iUtforelse.sub.tegninger.Arkitekttegninger, 2)
  sjekk('Utførelse: økonomi hører til anbud → 0', iUtforelse.cat.okonomi, undefined)

  const iAnbud = summer(rader, true, 'anbud')
  sjekk('Anbud: tegninger teller egen fase + uten fase', iAnbud.cat.tegninger, 2)
  sjekk('Anbud: økonomi teller 1', iAnbud.cat.okonomi, 1)

  const iFdv = summer(rader, true, 'fdv')
  sjekk('FDV: bare fila uten fase', iFdv.cat.tegninger, 1)

  // Prosjekt UTEN dokumentmal skal telle alt — det finnes ingen faser å filtrere på.
  const utenMal = summer(rader, false, null)
  sjekk('uten faser: teller alle rader', utenMal.cat.tegninger, 4)
  sjekk('uten faser: økonomi også', utenMal.cat.okonomi, 1)

  // Før fiksen ville alle fanene vist 4 her. Det var hele feilen.
  antall++
  if (iUtforelse.cat.tegninger !== 4 && iAnbud.cat.tegninger !== 4) {
    console.log('  ✓ ingen fane viser lengre summen av alle faser')
  } else { feil++; console.error('  ✗ telleren summerer fortsatt på tvers av faser') }
}

// Rådataene må tas vare på — ellers kan ikke summen regnes om ved fasebytte.
antall++
if (/const \[countRows, setCountRows\] = useState\(\[\]\)/.test(src)) {
  console.log('  ✓ rådataene fra RPC beholdes (summen utledes)')
} else {
  kritisk('countRows finnes ikke — er tellerne lagret ferdig summert igjen?')
}

antall++
if (/\}, \[countRows, hasFaser, viewedPhase\]\)/.test(src)) {
  console.log('  ✓ summen regnes om når fasen endres')
} else {
  kritisk('useMemo mangler viewedPhase i dependencies — tallet ville frosset ved fasebytte')
}
